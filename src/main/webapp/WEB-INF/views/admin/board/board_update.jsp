              ...
              <form name="form_write" method="post" action="/admin/board/board_update" enctype="multipart/form-data">
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <select name="board_type" class="form-control float-left">
                  ...
                  <div class="form-group">
                    ...
                    <input value="${boardVO.writer}" name="writer" type="text" class="form-control" id="writer" placeholder="제목을 입력해주세요">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputFile">첨부파일</label>
                    <c:forEach var="idx" begin="0" end="1">
                     <div class="input-group div_file_delete">
                     <!-- 위 div_file_delete 영역 이름은 첨부파일을 개별 Ajax 삭제할때 필요 -->
                      ...
                      <!-- 기존 업로드된 파일을 수정폼에 보여주기, 삭제버튼 필요 -->
                      <c:if test="${boardVO.save_file_names[idx] != null}">
                      	<p class="text-muted">
                      	<a href="/download?save_file_name=${boardVO.save_file_names[idx]}&real_file_name=${boardVO.real_file_name[idx]}">
...
<!-- 서머노트 웹 에디터 실행(아래 -개발자가 처리) -->
<script>
  $(document). ready(function(){
      // $('#content_lbl').summernote(); // 기본 실행. 이 기본실행을 개발자가 커스터마이징 합니다.
      $('#content').summernote({
          height: 150,
          lang: 'ko-KR',
          ...
          fontNames: ['Arial', 'Araial Black', 'Nanum Gothic','맑은고딕', '궁서', '굴림체', '굴림', '돋움체'],
