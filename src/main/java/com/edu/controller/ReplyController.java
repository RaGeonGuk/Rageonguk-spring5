...
public class ReplyController {
	...
	//댓글은 Read가 필요없음. 왜냐면 Select로 가져온 값을 Ajax로 처리하기 때문에 쿼리를 날릴 필요가 없습니다.
	//그래서, 바로 Update를 처리합니다. -간단하게 update시 Read 쿼리가 없고, Ajax처리함
	...
	public ResponseEntity<String> reply_update(@RequestBody ReplyVO replyVO) {
		//@RequestBody jsp에서 $.ajax로 보내는 데이터값 <-> @ResponseBody		
		...
		return result; //Restful 방식은 항상 body 반환값이(Body:String, Hasmap-json부분) 존재합니다.
	}
