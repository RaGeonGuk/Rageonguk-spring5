...
public class BoardServiceImpl implements IF_BoardService{
	...
	public void deleteBoard(int bno) throws Exception {
		// TODO 게시물삭제할때, 3개의 메서드가 실행(댓글 + 첨부파일 삭제 -> 게시물이 삭제됨)
		//트래젝션이 필요한 순간, 작업순서1. 첨부파일 삭제 OK 2. 게시물 삭제시 에러가 있다면 삭제가 안됨
		//위와 같은 상황을 방지하는 목적의 @Transantional 애노테이션을 사용합니다.
		// 댓글 삭제도 *나중에 추가
		//특이사항: 첨부파일 DB만 삭제해서 해결+실제 업로드 된 파일을 삭제가 필요 *나중에 추가
	...
	public void insertBoard(BoardVO boardVO) throws Exception {
		// TODO [부모] 게시물 insertBaord -> [자식] 첨부파일 있으면 첨부파일inserAttach첨부파일 있으면 첨부파일 insertAttach -> 게시물 insertBoard 실행
		//게시물 등록
		int bno = boardDAO.insertBoard(boardVO);
		boardDAO.insertBoard(boardVO);
		//첨부파일 등록: 1개 이상일대 가정해서 처리
